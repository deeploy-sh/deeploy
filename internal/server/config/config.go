...
// getDBConnectionDefault returns the SQLite connection string.
// Docker containers have /.dockerenv → use absolute path for host mount (/opt/deeploy/data:/data)
// Local development → use relative path (./data/)
func getDBConnectionDefault() string {
	if _, err := os.Stat("/.dockerenv"); err == nil {
		return "/data/deeploy.db?_pragma=foreign_keys(1)&_pragma=journal_mode(WAL)"
	}
	return "./data/deeploy.db?_pragma=foreign_keys(1)&_pragma=journal_mode(WAL)"
}
